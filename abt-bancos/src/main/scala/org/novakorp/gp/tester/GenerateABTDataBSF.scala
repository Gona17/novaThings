...
class GenerateABTDataBSF extends SparkSessionWrapper with DefinedFunctions {
  ...
  def generateCSV(df: DataFrame): Unit = {
    ...
    df.repartition(1)
      ...
      .option("delimiter","|")
      .option("header","true")
      .csv("hdfs://nameservice1/user/grodriguez/abt_bsf")

  }

  def fillABT(df: DataFrame, fecha_proceso: String): Unit = {

    df.withColumn("fecha_proceso", lit(s"${fecha_proceso}")).write.mode("overwrite").insertInto("de_bsf_3ref.abt_perfil_financiero")

    //    df.createOrReplaceTempView("abtTemp")
//    spark.sql(""" INSERT OVERWRITE de_bsf_3ref.abt_perfil_financiero AS
