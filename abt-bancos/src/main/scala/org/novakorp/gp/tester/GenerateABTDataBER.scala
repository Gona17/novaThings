...
class GenerateABTDataBER extends SparkSessionWrapper with DefinedFunctions {
  ...
  def generateCSV(df: DataFrame): Unit = {
    ...
    df.repartition(1)
      ...
      .option("delimiter","|")
      .option("header","true")
      .csv(s"hdfs://nameservice1/user/grodriguez/abt_ber")

  }

  def fillABT(df: DataFrame,fecha_proceso: String): Unit = {

    df.withColumn("fecha_proceso", lit(s"${fecha_proceso}")).write.mode("overwrite").insertInto("de_ber_3ref.abt_perfil_financiero")
